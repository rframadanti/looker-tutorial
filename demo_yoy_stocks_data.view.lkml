view: demo_yoy_stocks_data {
  ...
  derived_table: {
    sql:
    SELECT
    "selected_year" as period, a.*,
    c.stock_name as company, c.sector as sector
    ...
    UNION ALL
    SELECT
    "n_year_ago" as period, b.*,
    d.stock_name as company, d.sector as sector
    FROM dramadanti.demo_all_stocks b
    LEFT JOIN dramadanti.stock_lookup d ON b.name = d.stock_symbol
    WHERE b.ymd >= date_add(cast({% date_start date_filter %} as timestamp), -{% parameter yoy_reference %})
    AND b.ymd < date_add(cast({% date_end date_filter %} as timestamp), -{% parameter yoy_reference %})
